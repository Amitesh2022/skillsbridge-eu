package com.amitesh.portfolio.skillsbridgeeu.service;

import com.amitesh.portfolio.skillsbridgeeu.api.CreateRecordRequest;
...
public class PortfolioService {
    private final CopyOnWriteArrayList<PortfolioRecord> records = new CopyOnWriteArrayList<>(List.of(
            new PortfolioRecord(UUID.randomUUID(), "Skills profile review", "IN_REVIEW", "Learning task", Instant.now()),
            new PortfolioRecord(UUID.randomUUID(), "Programme approval", "READY", "Learning task", Instant.now()),
            new PortfolioRecord(UUID.randomUUID(), "Cohort invitation", "OPEN", "Learning task", Instant.now())
    ));
